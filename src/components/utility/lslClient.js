    ...
    _parse(message) {
        ...
        if (!this.metadata?.id) {
            if ("info" in data) {
                ...
                if ([1, 2, 4, 5, 6].includes(data.info.channel_format)) {
                    this.metadata.id = (typeof data.info?.source_id == "undefined") ? data.info.name : data.info.name + data.info.source_id;
                    ...
                    console.log("The LSL stream's data type cannot be handled by this app.")
                }
            } else {
                this.socket.close()
            }
        ...
        const res = {}
        for (var i = 0; i < this.metadata.channel_count; i++) {
            const val = data.data[data.data.length - 1];
            res["Channel " + (i + 1)] = val[i];
        }
