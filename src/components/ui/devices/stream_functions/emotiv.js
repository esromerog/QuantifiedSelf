...
export function EmotivConnection({ show, handleClose }) {
    ...
    const ids = useSelector((state)=>state.dataStream);
    
    const dataStreamRef = React.useRef(ids);
    dataStreamRef.current = ids;
    ...
    function handleActive() {
        ...
        setDisabled(true);
        setTimeout(() => {
            if (dataStreamRef.current["EMOTIV"] !== undefined) {
