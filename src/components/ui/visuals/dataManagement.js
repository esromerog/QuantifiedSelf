...
function ParameterDropDown({ claves, parameter, dataMappings, displayName }) {
    ...
    return (
        <div>
            <button type="button" className={dispColor} data-bs-toggle="dropdown" data-bs-auto-close="true" aria-expanded="false" onMouseEnter={() => setShow(true)}>Mapping - {display}</button>
...
function ParameterManager({ claves, parameter, dataMappings }) {
    ...
    const dropDown = (<ParameterDropDown
        claves={claves}
        parameter={parameter}
